...
int GetSlopeByDepthRange(const PsDeviceHandle device, const uint32_t sessionIndex, const PsDepthRange depthRange);
...
int main(int argc, char *argv[])
{
	...
	PsWDROutputMode wdrMode = { PsWDRTotalRange_Two, PsMidRange, 1, PsXFarRange, 1, PsUnknown, 1 };
...
GET:
	...
	if (dataMode == PsWDR_Depth)
	{
		...
		{
			wdrRange1Slope = GetSlopeByDepthRange(deviceHandle, sessionIndex, wdrMode.range1);
			cout << "wdrRange1Slope   =  " << wdrRange1Slope << endl;
			wdrRange2Slope = GetSlopeByDepthRange(deviceHandle, sessionIndex, wdrMode.range2);
			cout << "wdrRange2Slope   =  " << wdrRange2Slope << endl;
			if (wdrMode.totalRange == PsWDRTotalRange_Three)
			{
				wdrRange3Slope = GetSlopeByDepthRange(deviceHandle, sessionIndex, wdrMode.range3);
				cout << "wdrRange3Slope   =  " << wdrRange3Slope << endl;
			}
		}
	}
	else
	{
		...
		slope = GetSlopeByDepthRange(deviceHandle, sessionIndex, depthRange);
	...
	KEY:
		...
		if (key == 'M' || key == 'm')
		{
			...
			else if(PsWDR_Depth == t_datamode)
			{
				wdrRange1Slope = GetSlopeByDepthRange(deviceHandle, sessionIndex, wdrMode.range1);
				cout << "wdrRange1Slope   =  " << wdrRange1Slope << endl;
				wdrRange2Slope = GetSlopeByDepthRange(deviceHandle, sessionIndex, wdrMode.range2);
...
void Opencv_Depth(uint32_t slope, int height, int width, uint8_t*pData, cv::Mat& dispImg)
{
 ...
 int GetSlopeByDepthRange(const PsDeviceHandle device, const uint32_t sessionIndex, const PsDepthRange depthRange)
 {
	 ...
	 PsReturnStatus status = Ps2_GetMeasuringRange(device, sessionIndex, depthRange, &measuringrange);
	 if (status != PsReturnStatus::PsRetOK)
	 {
		 switch (depthRange)
		 {
